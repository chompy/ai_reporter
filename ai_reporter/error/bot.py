...
class MalformedBotResponseError(Exception):
    ...
    def retry_message(self) -> ChatCompletionUserMessageParam:
        return ChatCompletionUserMessageParam(
            role="user",
            content="I'm sorry, I couldn't understand your response, or something was missing. Please try again."
        )
...
class ToolPropertyInvalidError(ValueError, MalformedBotResponseError):
    ...
    def __init__(self, tool_name : str, property_name : str) -> None:
        ...
        super().__init__("property '%s' is invalid for tool '%s'" % (self.property_name, self.tool_name, ", %s"))

    def retry_message(self):
        return ChatCompletionUserMessageParam(
            role="user",
            content="Property '%s' was invalid in your call to the '%s' tool.  Please try again." % (
...
class ToolPropertyMissingError(AttributeError, MalformedBotResponseError):
    ...
    def retry_message(self):
        return ChatCompletionUserMessageParam(
            role="user",
            content="A required property, '%s', was missing in your call to the '%s' tool. Please try again." % (
                self.property_name, self.tool_name)
        )
...
class ToolNotDefinedError(MalformedBotResponseError):
    ...
    def retry_message(self):
        return ChatCompletionUserMessageParam(
            role="user",
            content="You tried to call a non-existent tool, '%s'. Please try again." % self.tool_name
        )
